...
import { dxt1_compress_block } from "./bc.js";

function ImageData_to_DXT1_ONEBITALPHA(img, { dither=true, alphaThreshold=127 } = {}) {
    if(img.width % 4 != 0 || img.height % 4 != 0) {
        console.error(`Cannot convert image to DXT1_ONEBITALPHA format! Image with or height isn't a multiple of 4!`);
        return;
    }

    const dxt1 = new Uint8Array((img.width * img.height) / 2);

    const threshold = alphaThreshold / 255;
    ...
    for(let y=0; y < img.height; y += 4) {
        for(let x=0; x < img.width; x += 4) {
            ...
            dxt1.set(dxt1_compress_block(block, dither, threshold), dxt1pos);
            dxt1pos += 8;
        }
    }
...
function DXT1_ONEBITALPHA_to_ImageData(dxt1, width, height) {
...
function DXT1_ONEBITALPHA_size(width, height) {
    if(width % 4 != 0 || height % 4 != 0) return null;
    return (width * height) / 2;
}

export { ImageData_to_DXT1_ONEBITALPHA, DXT1_ONEBITALPHA_to_ImageData, DXT1_ONEBITALPHA_size };
